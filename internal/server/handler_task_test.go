...
func TestLatencyRequestPipelineDTO(t *testing.T) {
	...
	err := json.Unmarshal([]byte(`{
		...
		"options":{"enableTLS":false,"maxResults":0,"probeCount":5,"httpProbeCount":2},
		"enableSpeed":true,
	...
	if req.Options.ProbeCount == nil || *req.Options.ProbeCount != 5 {
		t.Fatal("工作台 TCP 探测次数没有进入延迟请求 DTO")
	}
	if req.Options.HTTPProbeCount == nil || *req.Options.HTTPProbeCount != 2 {
		t.Fatal("工作台 HTTP 校验次数没有进入延迟请求 DTO")
	}
...
func TestOptionDTOExplicitZeroAndFalse(t *testing.T) {
	latency := (&latencyOptionsDTO{
		MaxResults: intPtr(0), ProbeCount: intPtr(5), HTTPProbeCount: intPtr(2), EnableTLS: boolPtr(false),
	...
	if latency.ProbeCount != 5 {
		t.Fatalf("工作台 TCP 探测次数覆盖失败: %+v", latency)
	}
	if latency.HTTPProbeCount != 2 {
		t.Fatalf("工作台 HTTP 校验次数覆盖失败: %+v", latency)
	}
	defaults := (&latencyOptionsDTO{}).apply(engine.DefaultLatencyOptions())
	if defaults.ProbeCount != 4 || defaults.HTTPProbeCount != 1 {
		t.Fatalf("default latency probes must be tcp=4 http=1: %+v", defaults)
	}
