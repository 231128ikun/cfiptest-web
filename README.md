...
- **准备候选**：反代模式支持本地文件、远程 TXT、粘贴输入，并按端口或备注筛选；加入候选时自动规范化、校验、去重并丢弃备注。官方模式独立提供 IPv4 / IPv6 网段抽样。
- **检测规则**：统一设置 TLS、延迟并发/超时/上限、速度并发/时限/下限，以及“符合全部条件的最大数量”；可恢复推荐设置。
- **检测及结果**：初次检测可选择“延迟检测后继续测速”，也可先测延迟，再对当前展示或勾选结果补测速；支持动态字段、组合筛选、排序和每组前 N。
- **直接导出**：默认导出全部结果，也可导出当前展示或已勾选结果。TXT 默认模板为 `{ip}:{port}#{country}`，支持自定义并保存到浏览器；CSV 直接复用结果表选择的字段。
...
已经完成检测的部分结果仍会保留在结果表；自动测速尚未完成的行会保留延迟元数据，速度留空。
...
是否自动执行“延迟 → 速度”流水线：未勾选时先产出延迟结果；勾选后只有同时满足
延迟与速度条件的节点才进入结果表。统一最大数量只在最终符合全部启用条件时计数。
...
仓库维护约定见 [CONTRIBUTING.md](CONTRIBUTING.md)，正式发布步骤见
