...
- **检测及结果**：支持动态字段、筛选和排序；显示字段支持全选、恢复默认并保存到 `data/settings.json`；“组合规则取前 N”可按国家、端口、ASN、数据中心等字段多选叠加。
- **直接导出**：默认导出全部结果，也可导出当前展示或已勾选结果。TXT 默认模板为 `{ip}:{port}#{country}`，支持自定义并保存到本地；CSV 直接复用结果表选择的字段。
...
- **检测设置持久化** —— `data/settings.json` 保存延迟/测速规则、显示字段、组合前 N 规则、导出范围与自定义模板。
