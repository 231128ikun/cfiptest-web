...
- **IP 库**（`data/ipdb/`）：支持多个命名库（新建 / 改名 / 删除 / 清空），库内容表格支持搜索筛选、分页与显示字段选择；手动导入仅支持 CSV 全字段文件（选择文件或拖拽，自动识别表头并保留国家/城市/ASN/延迟/速度等全部字段）；导出复用工作台导出逻辑（TXT 模板 / CSV 全字段）。首次启动自动把旧版 `data/ipdb.jsonl` 迁移为「默认库」。
- **自动维护**（`data/tasks.json`）：任务卡片（Master-Detail）。维护来源三选一：官方 IP 段（本地缓存，失效不删除）、远程 URL 库、本地 IP 库；可设：输入订阅文件（自动解析 `ip:port#备注` 导入库）、输出文件与模板（复用导出页内置/我的模板）、总数限制、测速总开关，以及多条规则。
...
维护流程：**按规则缺口取候选 → 延迟检测 → 条件过滤 → 必要时测速 → 达标入选**。每个规则达到配额后立即停止为它取候选；只有仍有缺口的规则继续检测，全部达标、任务总数达到上限或候选耗尽时结束。候选耗尽时按实际缺少条数报告。

本地库会更新实际检测结果；延迟连续失败达到设置阈值后才移除，未达到阈值以及测速失败的条目留库待复检。官方 IP 段和远程 URL 为只读来源，不写回、不删除。
